...
int main(int argc, char* argv[]) {
	// make a target graph
	enum { R, S, T, X, Y, Z, N }; // N is number of vertices
	Graph g(N);
	g[R].name = "r";
	g[S].name = "s";
	g[T].name = "t";
	...
	g[Y].name = "y";
	g[Z].name = "z";
	add_edge(R, S, EdgeProp { 5 }, g);
	add_edge(R, T, EdgeProp { 3 }, g);
	add_edge(S, T, EdgeProp { 2 }, g);
	add_edge(S, X, EdgeProp { 6 }, g);
	add_edge(T, X, EdgeProp { 7 }, g);
	add_edge(T, Y, EdgeProp { 4 }, g);
	add_edge(T, Z, EdgeProp { 2 }, g);
	add_edge(X, Y, EdgeProp { -1 }, g);
	add_edge(X, Z, EdgeProp { 1 }, g);
	...
	Visitor vis;
	dag_shortest_paths(g, vertex(S, g),
