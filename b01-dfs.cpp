...
void my_dfs_rec(const Graph& g, int s, std::deque<bool>& visit) {
	...
	for (auto it = vp.first; it != vp.second; ++it) {
		...
		if (visit[ind] == false) {
			my_dfs_rec(g, ind, visit);
		}
	}
}

void my_dfs(const Graph& g, int s) {
	int n = num_vertices(g);
	std::deque<bool> visit(n, false); // vert_ind -> visited or not
	my_dfs_rec(g, s, visit);
}

void my_dfs(const Graph& g) {
	int n = num_vertices(g);
	std::deque<bool> visit(n, false); // vert_ind -> visited or not
	for (int i = 0; i < visit.size(); ++i) {
		if (visit[i] == false) {
			cout << "( ";
			my_dfs_rec(g, i, visit);
			cout << ") ";
		}
	}
}
