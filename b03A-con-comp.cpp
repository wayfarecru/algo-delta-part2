...
void my_dfs_prop_rec(Graph& g, const VertDesc& vd) {
	...
	for (auto it = adjp.first; it != adjp.second; ++it) {
		if (vertColorMap[*it] == boost::white_color) {
			my_dfs_prop_rec(g, *it);
		}
	}
	vertColorMap[vd] = boost::black_color; // finished
}

void my_dfs_prop(Graph& g, int s) {
	VertColorMap vertColorMap = boost::get(boost::vertex_color, g);
	...
	// recursive visit
	my_dfs_prop_rec(g, boost::vertex(s, g));
}

void my_dfs_prop(Graph& g) {
	VertColorMap vertColorMap = boost::get(boost::vertex_color, g);
	auto vp = boost::vertices(g);
	// reset all vertex colors to white
	for (auto it = vp.first; it != vp.second; ++it) {
...
int main(int argc, char* argv[]) {
	...
	int M = static_cast<int>(N * sqrt(sqrt(sqrt(N)))); // num edges
	// make a random graph
	...
	cout << "boost::connected_component: " << endl;
	std::vector<int> comp(boost::num_vertices(g));
	int num = boost::connected_components(g, &comp[0]);
	cout << "total number of components = " << num << endl;
	if (argc == 1) {
		for (int i = 0; i < comp.size(); ++i) {
			cout << "vertex " << i << " is in component " << comp[i] << endl;
		}
	} else {
		for (int i = 0; i < min(8, (int)(comp.size())); ++i) {
			cout << "vertex " << i << " is in component " << comp[i] << endl;
		}
		cout << ". . ." << endl;
		for (int i = max(8, (int)(comp.size() - 8)); i < comp.size(); ++i) {
			cout << "vertex " << i << " is in component " << comp[i] << endl;
		}
	}
	// done
