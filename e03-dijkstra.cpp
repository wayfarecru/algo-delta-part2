...
int main(int argc, char* argv[]) {
	// make a target graph
	enum { S, T, X, Y, Z, N }; // N is number of vertices
	Graph g(N);
	g[S].name = "s";
	g[T].name = "t";
	g[X].name = "x";
	g[Y].name = "y";
	g[Z].name = "z";
	add_edge(S, T, EdgeProp { 10 }, g);
	add_edge(S, Y, EdgeProp { 5 }, g);
	add_edge(T, X, EdgeProp { 1 }, g);
	add_edge(T, Y, EdgeProp { 2 }, g);
	add_edge(X, Z, EdgeProp { 4 }, g);
	add_edge(Y, T, EdgeProp { 3 }, g);
	add_edge(Y, X, EdgeProp { 9 }, g);
	add_edge(Y, Z, EdgeProp { 2 }, g);
	add_edge(Z, S, EdgeProp { 7 }, g);
	add_edge(Z, X, EdgeProp { 6 }, g);
	my_print(g);
	// Dijkstra shortest-path
	cout << "Dijkstra shortest-path:" << endl;
	g[0].dist = 0; // set start vertex 0
	for (int i = 1; i < N; ++i) {
		g[i].dist = std::numeric_limits<int>::max();
	}
	Visitor vis;
	dijkstra_shortest_paths(g, vertex(S, g),
